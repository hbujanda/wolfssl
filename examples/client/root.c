...
#define FS_VOLUME1_DIR                  FS_VOLUME1 "/"
    ...
    /* Following call will try to connect to a server application
       started with 'server -b -d -c server.crt -k server.key' arguments 
       on the specified ip
     */
    {
        char *argv[] = {"client", "-h", "192.168.2.134", "-A", "FLASH0/ca.crt"};
